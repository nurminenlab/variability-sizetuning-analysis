...
SG_eigenvalues = NaN * ones(n_penetrations,n_eigen,n_diameters);
G_eigenvalues  = NaN * ones(n_penetrations,n_eigen,n_diameters);
IG_eigenvalues = NaN * ones(n_penetrations,n_eigen,n_diameters);
...
for i = 1:length(penetrations)
    ...
    for stim = 1:length(fls)
        ...
        for l = 1:3
            ...
            results = neuralTraj(rindx,D,size(D(1).spikes,1));
            ...
            bsl_results = neuralTraj(rindx,D_bsl,size(D(1).spikes,1));
            ...
            elseif l == 2
                ...
                G_eigenvalues(i,1:size(D(1).spikes,1),si) = E;
            else
                ...
                IG_eigenvalues(i,1:size(D(1).spikes,1),si) = E;
            end            
