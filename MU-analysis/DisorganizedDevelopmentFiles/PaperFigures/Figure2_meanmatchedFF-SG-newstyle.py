...
FA_SML, FA_RF, mean_SML, mean_RF  = dalib.mean_match_FA(mn_SML, 
                                                        ...
                                                        FF_RF, 
                                                        count_bins)
count_bins = np.arange(0,100,0.5)
...
ax.bar([0,1], FFs, yerr=ffSEs_RFLAR, color=['grey','orange'], width=0.5)
ax.set_xticks([0,1])
ax.set_xticklabels(['0.2','0.4'])
ax.set_ylabel('Fano factor')

ax = plt.subplot(2,2,4)
ax.bar([0,1], FFs_RFLAR, yerr=ffSEs_RFLAR, color=['orange','blue'], width=0.5)
ax.set_xticks([0,1])
ax.set_xticklabels(['0.4','26'])

if save_figures:
    plt.savefig(fig_dir + 'SG_mean_match_bars.svg')
